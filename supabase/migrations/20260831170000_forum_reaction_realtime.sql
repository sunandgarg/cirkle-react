-- Fan out forum reaction counts through the same durable delivery path as
-- messages. The payload deliberately omits viewer-specific myReactions so
-- each browser keeps its own local state while receiving shared totals.
...
begin
  ...
  values ('forum', v_post.id, v_channel, 'UPDATE',
...
drop trigger if exists enqueue_forum_reaction_realtime_trigger on public.reactions;
create trigger enqueue_forum_reaction_realtime_trigger
after insert or delete on public.reactions
for each row execute function public.enqueue_forum_reaction_realtime();
