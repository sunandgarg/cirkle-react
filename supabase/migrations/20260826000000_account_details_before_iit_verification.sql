...
alter table public.profiles add column if not exists phone_full text;

alter table public.profiles drop constraint if exists profiles_phone_number_digits;
alter table public.profiles add constraint profiles_phone_number_digits
  check (phone_number is null or phone_number ~ '^[0-9]{10}$');

grant update (phone_country_code, phone_number, phone_full) on table public.profiles to authenticated;
...
begin
  ...
  if length(v_name) < 2 then raise exception 'A valid name is required'; end if;
  ...
  insert into public.profiles (
    ...
    v_user_id,
    v_name,
    case when v_phone = '' then null else v_country end,
    nullif(v_phone, ''),
...
begin
  ...
  update public.profiles set
    ...
    phone_country_code = case when v_phone = '' then phone_country_code else v_country end,
    phone_number = case when v_phone = '' then phone_number else v_phone end,
    phone_full = case when v_phone = '' then phone_full else v_country || v_phone end,
