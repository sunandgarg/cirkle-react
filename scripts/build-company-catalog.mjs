...
let preservedRecruiters = [];
try {
  preservedRecruiters = JSON.parse(await readFile(publicCatalogFile, "utf8"))
    .filter((company) => company?.iitRecruiter === true && Number(company.rank) > rankedCompanyLimit + customCompanies.length);
} catch {
  preservedRecruiters = [];
}
const catalogCompanies = [
  ...[...rankedCompanies, ...customCompanies].map(({ rank, name, ticker, logo }) => ({ rank, name, ticker, logo })),
  ...preservedRecruiters,
];
...
const generated = `// Generated by scripts/build-company-catalog.mjs from the public market-cap ranking.\n` +
  ...
  `export const topCompanies = JSON.parse(${JSON.stringify(JSON.stringify(catalogCompanies))}) as TopCompany[];\n`;
