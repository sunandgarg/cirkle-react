...
The browser compresses ordinary images to WebP and iteratively targets at most 0.5 MiB before upload. The server independently decodes every claimed image, strips its original encoding/metadata, converts it to WebP, and enforces the same stored-byte cap. Source images may be at most 10 MiB so a 1 MiB photo can still be compressed; PDFs, Office documents, text, audio, and video must already be 0.5 MiB or smaller. Arbitrary PDF/Office “extreme compression” is deliberately not attempted because it can destroy text, signatures, accessibility, or document structure; oversize files fail clearly instead. Published private content uses immutable object keys: a different payload cannot overwrite an existing key, and referenced content cannot be independently deleted while its post, message, story, logo, or verification record remains active.

CloudFront signed URLs and Origin Access Control are implemented in `aws/hosting/cirkle-react-media.yaml` and the API storage service. A fresh 8 September 2026 deployment attempt still returned `403 Your account must be verified before you can add new CloudFront resources`; CloudFormation rolled back cleanly and the new AWS request ID was added to the existing account-verification Support case. The deployed stack therefore continues to use the safe private-S3/API-streaming fallback. Do not stop/delete the legacy EC2 rollback host until AWS removes the hold, CloudFront is deployed, signed public/private delivery passes, and backup-bucket ownership is decoupled. Do not make the bucket public as a workaround.
...
Zavu is the fail-closed delivery route for every supported IIT student/alumni
domain, including IIT addresses entered on the main login screen. Institute
verification OTP is always forced through the verified `verify@cirkle.world`
Zavu sender; non-IIT destinations continue through ZeptoMail. Neither provider
